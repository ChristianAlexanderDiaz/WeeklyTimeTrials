"""
Accept duel command for the MKW Time Trial Bot.

This command allows users to accept a pending duel invitation.
"""
...
class AcceptDuelCommand(AutocompleteCommand):
    """
    Command to accept a pending duel invitation.

    This command handles:
    - Finding pending duels for the user
    - Validating duel selection
    - Updating duel status to 'active'
    - Notifying the creator
    """

    async def execute(self, interaction: Interaction, challenge_number: int) -> None:
        """
        Execute the accept duel command.

        Args:
            interaction: Discord interaction object
            challenge_number: Challenge number to accept
        """
        ...
        if not duel_data:
            raise CommandError(
                ...
                f"Use `/accept-duel` autocomplete to see your pending invitations."
            )

        # Verify user is the opponent
        if duel_data['opponent_user_id'] != user_id:
            raise CommandError("You can only accept duels where you are the challenged opponent.")

        # Accept the duel (update to active status)
        await self._accept_duel(duel_data['id'])
        ...
        embed = DuelFormatter.create_duel_accepted_embed(
            duel_data=duel_data,
            creator_name=creator_name,
            opponent_name=opponent_name
        )

        # Send response and ping creator
        creator = await interaction.guild.fetch_member(duel_data['creator_user_id'])
        await self._send_response(
            interaction,
            content=f"{creator.mention}, your challenge has been accepted!",
            embed=embed,
            ephemeral=False
        )
    ...
    async def _accept_duel(self, challenge_id: int) -> None:
        """
        Accept a duel by updating its status to active.

        Args:
            challenge_id: Challenge ID

        Raises:
            CommandError: If acceptance fails
        """
        query = """
        ...
        if not results:
            raise CommandError("Failed to accept duel. It may have already been accepted or cancelled.")
...
def setup_accept_duel_command(tree: app_commands.CommandTree) -> None:
    """
    Set up the accept duel command with the Discord command tree.

    Args:
        tree: Discord app commands tree
    """
    accept_duel_cmd = AcceptDuelCommand()

    @tree.command(
        name="accept-duel",
        description="Accept a pending duel invitation"
    )
    @app_commands.describe(
        challenge_number="Select the duel to accept (autocomplete shows your pending invitations)"
    )
    async def accept_duel(interaction: Interaction, challenge_number: int):
        """
        Accept a pending duel invitation.

        Examples:
        /accept-duel challenge_number:1
        """
        await accept_duel_cmd.handle_command(interaction, challenge_number=challenge_number)

    @accept_duel.autocomplete('challenge_number')
    async def challenge_autocomplete(interaction: Interaction, current: str) -> List[app_commands.Choice[int]]:
        ...
        return await accept_duel_cmd.autocomplete_callback(interaction, current)
